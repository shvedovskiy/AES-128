...
{
    ui   -> setupUi(this);
    this -> setWindowTitle("AES-128 Text Encryption");
...
{
    int returncode = aes.InputForEncrypt(ui -> lineEditInput -> toPlainText().toStdString(),
...
{
    int returncode = aes.InputForDecrypt(ui -> lineEditEncrypted -> toPlainText().toStdString(),
                                         ui -> lineEditKey       -> text().toStdString()
                                         );
